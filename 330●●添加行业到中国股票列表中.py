...
with open('data/300_中国股票symbol信息.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
...
for kk in ddd3.keys():
    ...
    # 请求的URL
    url = f'https://stock.xueqiu.com/v5/stock/screener/quote/list.json?page=1&size=999999&order=desc&order_by=percent&market=CN&ind_code={kk}'
    ...
    for dd2 in ddd2["data"]["list"]:
        if dd2["symbol"] in ddd.keys():
            ddd[dd2["symbol"]].append(f"@{ddd3[kk]}")
        else:
            ddd[dd2["symbol"]]=[dd2["name"],f"@{ddd3[kk]}"]


with open(f'data/300_中国股票symbol信息.txt', 'w', encoding='utf-8', newline='\r\n') as f:
