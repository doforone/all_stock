...
while ddd!=[]:
    ...
    if 1:
        print(f"---{len(ddd)}---")
        #=================
        if os.path.exists(f'data/CN_前复权/{dd[0]}_d.txt'):
            with open(f'data/CN_前复权/{dd[0]}_d.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
...
&klt=101&fqt=1&end=20500101&lmt={lmt}&_={int(time.time()*1000)}"
            try:
                ...
                if lmt==FF:
                    with open(f'data/CN_前复权/{dd[0]}_d.txt', 'w', encoding='utf-8', newline='\r\n') as f:
...
with open(f'_new_data_CN1.txt', 'w', encoding='utf-8', newline='\r\n') as f:
...
# 启动第一个程序
# 这里假设第一个程序是 test1.py
os.system("python 110_●前复权数据CN1上传同步.py")
