...
while ddd!=[]:
    ...
    if 1:
        print(f"---{len(ddd)}---")
        #=================
        if os.path.exists(f'data/CN_不复权/{dd[0]}_d.txt'):
            with open(f'data/CN_不复权/{dd[0]}_d.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
        ...
        if typee!="":
            url=f"http://39.push2his.eastmoney.com/api/qt/stock/kline/get?cb=jQuery351030454019657600573_1707793477878\
&secid={typee}.{dd[0][2:]}&ut=fa5fd1943c7b386f172d6893dbfba10b&fields1=f1%2Cf2%2Cf3%2Cf4%2Cf5%2Cf6&fields2=f51%2Cf52%2Cf53%2Cf54%2Cf55%2Cf56%2Cf57%2Cf58%2Cf59%2Cf60%2Cf61\
&klt=101&fqt=0&end=20500101&lmt={lmt}&_={int(time.time()*1000)}"
            try:
                ...
                if lmt==FF:
                    with open(f'data/CN_不复权/{dd[0]}_d.txt', 'w', encoding='utf-8', newline='\r\n') as f:
                        f.write(json.dumps(ddd2, indent=0, ensure_ascii=False)+"\r\n")

                    nnn[dd[0]]=[]  # 写入到新数据文件里
                    #nnn[dd[0]].extend([[xx[0],round(xx[2]-xx[9],2),xx[1],xx[4],xx[3],xx[2],xx[5],xx[6],xx[10]] for xx in ddd2])
                    nnn[dd[0]].extend([[xx[0],xx[1],xx[4],xx[3],xx[2],xx[9],xx[8],xx[5],xx[6],xx[10]] for xx in ddd2])
                    ...
                    if ddd2!=[]:
                        if ddd2[-1]==uuu[-1]:  # 已有,就不再更新了
                            ...
                            if ddd2[0] in uuu and uuu[-2] in ddd2:  # -2是因为最后一个可能是实时的数据,不是最终的K线数据
                                ...
                                nnn[dd[0]]=[]  # 写入到新数据文件里
                                #nnn[dd[0]].extend([[xx[0],round(xx[2]-xx[9],2),xx[1],xx[4],xx[3],xx[2],xx[5],xx[6],xx[10]] for xx in ddd2[i+1:]])
                                nnn[dd[0]].extend([[xx[0],xx[1],xx[4],xx[3],xx[2],xx[9],xx[8],xx[5],xx[6],xx[10]] for xx in ddd2[i+1:]])
...
with open(f'_new_data_CN0.txt', 'w', encoding='utf-8', newline='\r\n') as f:
...
# 启动第一个程序
# 这里假设第一个程序是 test1.py
os.system("python 110_●不复权数据CN0上传同步.py")
