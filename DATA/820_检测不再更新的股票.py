...
for dd in ddd.keys():
    if os.path.exists(f'雪球_不复权\\{dd}_d.txt'):
        ...
        if uuu==[] or tt_d(int(uuu[-1][0]/1000))<"2024-01-01":
            nnn[dd]=None  # 不再更新的
        else:
            pass
    else:
...
with open(f'010_不再更新的全部股票.txt', 'w', encoding='utf-8', newline='\r\n') as f:
    f.write(json.dumps(nnn, indent=4, ensure_ascii=False)+"\r\n")
