...
while ddd!=[]:
    ...
    if dd not in ddd0.keys():  # 排除已不再更新的
    #if 1:
        #=================
        if os.path.exists(f'雪球_不复权/{dd}_d.txt'):
            with open(f'雪球_不复权/{dd}_d.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
        ...
        try:
            # 请求的URL
            url = f'https://stock.xueqiu.com/v5/stock/chart/kline.json?symbol={dd}&begin={int(time.time()*1000)}&period=day&type=normal&count=-{lmt}&indicator=kline,pe,pb,ps,pcf,market_capital,agt,ggt,balance'
            ...
            if "data" in vvv.keys() and "item" in vvv["data"].keys():
                ...
                if lmt==FF:
                    with open(f'雪球_不复权/{dd}_d.txt', 'w', encoding='utf-8', newline='\r\n') as f:
...
with open(f'_new_data_0.txt', 'w', encoding='utf-8', newline='\r\n') as f:
