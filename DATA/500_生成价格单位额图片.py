...
#for kk in ["VISN"]:
    ...
    with open(f'雪球_不复权/{kk}_d.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:  # 默认不复权
        uuu0=json.loads(f.read())[-1280*5:]
        uuu0=[[tt_d(int(uu[0]/1000)),f0(uu[2]),f0(uu[4]),f0(uu[3]),f0(uu[5]),f0(uu[1]),
               f0(uu[9]),f0(uu[8])] for uu in uuu0]
    with open(f'雪球_前复权/{kk}_d.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:  # 默认前复权
        uuu1=json.loads(f.read())[-1280*5:]
        uuu1=[[tt_d(int(uu[0]/1000)),f0(uu[2]),f0(uu[4]),f0(uu[3]),f0(uu[5]),f0(uu[1]),
               f0(uu[9]),f0(uu[8])] for uu in uuu1]

    if (lenn:=len(uuu0))==len(uuu1) and lenn>0 and uuu0[0][0]==uuu1[0][0]:
        uuu0=uuu0[::-1]  # 不复权
        uuu0=[uuu0[i:i+5] for i in range(0,lenn,5)]
...
##        hl_x=[[x[0], 0.0 if x[1]==0 else y[1]/x[1]] for x in hhll0 for y in aaa0]
        ...
        uuu1=uuu1[::-1]  # 前复权
        uuu1=[uuu1[i:i+5] for i in range(0,lenn,5)]
...
##        ttt1=[[x[0][0],sum([y[7] for y in x])] for x in uuu1]  # 换手率turnoverrate
        ...
        maxx0=max([x[1] for x in hl_x])
        maxx=max([x[1] if x[1]<maxx0 else 0 for x in hl_x])
        ...
        for vv in hl_x[::-1]:
            ...
            if vv[1]==None or vv[1]==0 or maxx_minn==0:
                ...
                if vv[1]<maxx0:
                    ...
                    draw.line((i, 312, i, 312+300), '#0033ff')
