...
while ddd!=[]:
    ...
    if 1:
        print(f"---{len(ddd)}---")
        #=================
        if os.path.exists(f'data/HK_不复权/{dd}_d.txt'):
            with open(f'data/HK_不复权/{dd}_d.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
        ...
        if typee!="":
            url=f"https://push2his.eastmoney.com/api/qt/stock/kline/get?cb=jQuery35102554458253436447_1707716799562\
...
&klt=101&fqt=0&end=20500101&lmt={lmt}&_={int(time.time()*1000)}"
            try:
                ...
                if lmt==FF:
                    with open(f'data/HK_不复权/{dd}_d.txt', 'w', encoding='utf-8', newline='\r\n') as f:
...
with open(f'_new_data_HK0.txt', 'w', encoding='utf-8', newline='\r\n') as f:
...
# 启动第一个程序
# 这里假设第一个程序是 test1.py
os.system("python 110_●不复权数据HK0上传同步.py")
