...
# https://stock.xueqiu.com/v5/stock/screener/quote/list.json?page=1&size=30&order=desc&order_by=percent&market=HK&type=hk&is_delay=true

rrr={}
for p in range(1,999999):
    time.sleep(1)
    url=f"https://stock.xueqiu.com/v5/stock/screener/quote/list.json?page={p}&size=30&order=desc&order_by=percent&market=HK&type=hk&is_delay=true"
    print(url)
    try:
        htmll=get_htmll(urll=url)
        uuu=json.loads(htmll)
        print(uuu)
        if uuu["data"]=={}:
            break
        else:
            for uu in uuu["data"]["list"]:
                if uu["symbol"] not in rrr.keys():
                    rrr[uu["symbol"]]=[uu["name"]]
    except Exception as e:
        print(e)
        print("--err")

print(len(rrr))
with open(f'data/香港股票symbol信息.txt', 'w', encoding='utf-8', newline='\r\n') as f:
