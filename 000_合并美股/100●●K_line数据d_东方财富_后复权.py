...
while ddd!=[]:
    ...
    if dd[2]==1:  # 可以更新的美股
        print(f"---{len(ddd)}---")
        #=================
        if os.path.exists(f'data/US_后复权/{dd[0]}_d.txt'):
            with open(f'data/US_后复权/{dd[0]}_d.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
...
klt=101&fqt=2&end=20500101&lmt={lmt}&_={int(time.time()*1000)}"
            try:
                ...
                if lmt==FF:
                    with open(f'data/US_后复权/{dd[0]}_d.txt', 'w', encoding='utf-8', newline='\r\n') as f:
...
with open(f'_new_data_US2.txt', 'w', encoding='utf-8', newline='\r\n') as f:
...
# 启动第一个程序
# 这里假设第一个程序是 test1.py
os.system("python 110_●后复权数据US2上传同步.py")
